...
describe('AsgardeoNextClient', () => {
  ...
  describe('switchOrganization', () => {
    ...
    it('rejects a successful response that carries no access token', async () => {
      mockTokenEndpoint({json: async (): Promise<unknown> => ({token_type: 'Bearer'})});

      await expect(client.switchOrganization(organization, 'session-1')).rejects.toThrow(/access_token/);
      expect(storageManager.setSessionData).not.toHaveBeenCalled();
    });
