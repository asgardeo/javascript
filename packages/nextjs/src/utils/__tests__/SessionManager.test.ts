...
describe('SessionManager', () => {
  ...
  describe('createSessionToken', () => {
    ...
    it('keeps only the essential claims when the full claim set does not fit into the cookie', async () => {
      const oversizedClaims: Record<string, unknown> = {
        ...idTokenClaims,
        email: 'jane@example.com',
        groups: Array.from({length: 200}, (): string => 'x'.repeat(30)),
      };
      ...
      expect(token.length).toBeLessThanOrEqual(SessionManager.getSessionCookieValueBudget());

      const payload: SessionTokenPayload = await SessionManager.verifySessionToken(token);

      expect(payload.idTokenClaims).toEqual({
        email: 'jane@example.com',
        org_id: 'org-1',
        org_name: 'Acme',
        sub: 'user-1',
    ...
    it('keeps only the organization claims when the essential claims do not fit into the cookie either', async () => {
      const bulkyEssentialClaims: Record<string, unknown> = {
        ...idTokenClaims,
        email: 'jane@example.com',
        given_name: 'g'.repeat(1200),
        name: 'n'.repeat(1200),
        org_handle: 'acme',
        user_org: 'org-1',
      };
    ...
    it('leaves the claims out when not even the organization claims fit into the cookie', async () => {
      const token: string = await SessionManager.createSessionToken(
        'a'.repeat(SessionManager.getSessionCookieValueBudget()),
        'user-1',
        'session-1',
        'openid',
        3600,
      ...
      expect(payload.accessToken).toHaveLength(SessionManager.getSessionCookieValueBudget());
      expect('idTokenClaims' in payload).toBe(false);
    });
  });

  describe('toOrganizationIdTokenClaims', () => {
    it('keeps the organization claims and the subject only', () => {
      expect(
        SessionManager.toOrganizationIdTokenClaims({
          email: 'jane@example.com',
          org_handle: 'acme',
          org_id: 'org-1',
          org_name: 'Acme',
      ...
      ).toEqual({org_handle: 'acme', org_id: 'org-1', org_name: 'Acme', sub: 'user-1', user_org: 'org-1'});
    });
  });

  describe('toEssentialIdTokenClaims', () => {
    it('keeps the organization and basic identity claims only', () => {
      expect(
        SessionManager.toEssentialIdTokenClaims({
          email: 'jane@example.com',
          groups: ['admins'],
          org_handle: 'acme',
          org_id: 'org-1',
