...
import {AsgardeoJavaScriptClient, OIDCRequestConstants} from '@asgardeo/javascript';
import {SignOutError} from '../models/sign-out-error';
import {
  ERROR,
  ERROR_DESCRIPTION,
...
export class SPAUtils {
  ...
  public static async isSignOutSuccessful(): Promise<boolean> {
    if (AsgardeoJavaScriptClient.isSignOutSuccessful(window.location.href)) {
  ...
  public static didSignOutFail(): boolean | SignOutError {
    if (AsgardeoJavaScriptClient.didSignOutFail(window.location.href)) {
