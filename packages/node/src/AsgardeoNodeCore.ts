...
export class AsgardeoNodeCore<T> {
  private auth: AsgardeoJavaScriptClient<T>;
  ...
  constructor(config: AuthClientConfig<T>, store?: Storage) {
    ...
    this.auth = new AsgardeoJavaScriptClient(config, this.store, this.cryptoUtils);
    this.storageManager = this.auth.getStorageManager();
    Logger.debug('Initialized AsgardeoJavaScriptClient successfully');
  }
  ...
  public async reInitialize(config: Partial<AuthClientConfig<T>>): Promise<boolean> {
